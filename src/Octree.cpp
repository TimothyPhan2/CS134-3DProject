...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	if (level >= numLevels || node.points.size() <= 1) return;  // Stop recursion if max depth is reached or node cannot be subdivided further

    vector<Box> childBoxes;
    subDivideBox8(node.box, childBoxes);  // Divide the current node's box into 8 parts

    // Create child nodes and distribute points to them
    for (int i = 0; i < 8; i++) {
        TreeNode childNode;
        childNode.box = childBoxes[i];
        vector<int> childPoints;
        int numPoints = getMeshPointsInBox(mesh, node.points, childNode.box, childPoints);
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
    ...
    if (node.children.empty()) {  // Leaf node
        nodeRtn = node;  // The node that is returned
        return true;
    } else {
        bool hit = false;
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn) {
	...
	if (node.box.overlap(box)) {
        if (!node.children.empty()) {
            for (auto &child : node.children) {
                intersect(box, child, boxListRtn);  // Recursively check for intersecting children
            }
        }else{
			intersects = true;
...
void Octree::draw(TreeNode & node, int numLevels, int level) {
	if (level >= numLevels) return;  // Only draw up to the specified level

    // Select a color based on the level
    float hue = 255 * float(level) / float(numLevels);
    ofSetColor(ofColor::fromHsb(hue, 255, 255));

    drawBox(node.box);  // Draw the current node's box
